...
class Postgres:
    ...
    def __refresh_cursor(self, pre_settings=True):
        # if self._connection is not None:
        #     try:
        #         self._connection.close()  # 显式关闭旧连接，释放DuckDB锁
        #     except Exception:
        #         pass  # 忽略关闭时的潜在异常
    ...
    def execute(self, sql, cache=False, retry_limit=None, fetch = True):
        ...
        res = tuple(self.__cur.fetchall()) if fetch else None
        if cache:
    ...
    def list_foreign_keys(self, multi_columns=False,schema='public'):
        sql = f'''select conname as fk_name, pg_class.relname as t_name, 
...
class Schema:
    def __init__(self, database : Postgres = None,schema = 'public'):
        if database is not None:
            db = database._connection
            tables = postgres.tables(database._connection)

            tables = list(filter(lambda x: x[0] == schema, tables))
            ...
            for i, (sname, tname) in enumerate(tables):
                ...
                index_query = f'select indexname, indexdef from pg_indexes ' \
                              f'where schemaname = \'{schema if sname is None else sname}\' and tablename = \'{tname}\';'
            ...
            for fk_id, (table_name, column_name), (fk_table_name, fk_column_name) in database.list_foreign_keys(schema=schema):
            ...
            for fk_id, (table_name, column_name), (fk_table_name, fk_column_name) in database.list_foreign_keys(multi_columns=True,schema=schema):
            ...
            self.pg_stats = {}
            pg_stats_query = f'select * from pg_stats where schemaname = \'{schema}\';'
